...
공개 페이지: GitHub Pages 배포 후 `https://<계정>.github.io/<저장소>/`
...
gh api -X POST repos/<계정>/tesla-tab-map/pages -f "build_type=legacy" -f "source[branch]=main" -f "source[path]=/"
```

또는 GitHub 웹: Settings → Pages → Branch `main` / folder `/ (root)` → Save.

수 분 후 `https://<계정>.github.io/tesla-tab-map/` 가 열립니다. 위치 권한은 HTTPS에서만 동작합니다.
